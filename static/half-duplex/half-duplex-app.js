...
function appendAssistantDelta(delta) {
    if (!_currentAssistantEntry) {
        ...
        _currentAssistantEntry._textSpan.className = 'ai-text';
...
function finalizeAssistantMessage() {
    if (_currentAssistantEntry && _currentAssistantEntry._textSpan) {
        const text = _currentAssistantEntry._textSpan.textContent;
        _currentAssistantEntry._textSpan.innerHTML = renderMarkdown(text);
    }
    _currentAssistantEntry = null;
}
...
function renderMarkdown(text) {
    ...
    html = html.replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
        const langMap = { py: 'python', js: 'javascript', sh: 'bash', shell: 'bash' };
        const langClass = lang && langMap[lang] ? langMap[lang] : (lang || 'plaintext');
        const codeEscaped = code.replace(/&lt;/g, '<').replace(/&gt;/g, '>');
        const highlighted = hljs.highlight(codeEscaped, { language: langClass }).value;
